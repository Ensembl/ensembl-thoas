...
def test_slice_formatting():
    ...
    """
    slice_dict = format_slice(
        "test_name", "test_code", True, 1, 100, 200, "test_genome"
    )
    ...
    slice_dict = format_slice(
        "test_name", "test_code", False, -1, 100, 200, "test_genome"
    )
...
def test_format_region():
    ...
    genome_id = get_genome_id(
        test_mysql_result["species_name"], test_mysql_result["accession_id"]
    )
    chromosome_checksums = ChromosomeChecksum(genome_id, "/test_path/")
    region = format_region(
        test_mysql_result, "test_assembly_id", genome_id, chromosome_checksums
    )
...
def test_phase_calculation():
    ...
    phase_lookup = {"ENST01": {"ENSE01": (-1, 0), "ENSE02": (0, -1)}}
    ...
    for i, phased_exon in enumerate(phased_exons):
        ...
        assert (phased_exon["start_phase"], phased_exon["end_phase"]) == (
...
def test_splice_formatting():
    ...
    transcript = {"start": 1, "end": 30, "strand": 1}

    exon_list = [
        {
            "slice": {"location": {"start": 1, "end": 10}},
            "stable_id": "ENSE01",
            "unversioned_stable_id": "ENSE01",
        },
        {
            "slice": {"location": {"start": 21, "end": 30}},
            "stable_id": "ENSE02",
            "unversioned_stable_id": "ENSE02",
        },
    ]
    ...
    assert splicing[0]["relative_location"] == {"start": 1, "end": 10, "length": 10}
    assert splicing[1]["index"] == 2
    assert splicing[1]["exon"] == exon_list[1]
    assert splicing[1]["relative_location"] == {"start": 21, "end": 30, "length": 10}
...
def test_utr_formatting():
    ...
    forward_cds = {
        ...
        "relative_start": 2,
        "relative_end": 10,
        "transcript": {"strand": 1, "start": 1, "end": 10},
        "downstream": False,
    }
    utr = format_utr(
...
def test_infer_introns():
    ...
    transcript = {"start": 10, "end": 100, "strand": 1}
    ...
    target_introns = [
        {
            ...
            "slice": {
                "region_id": "test_genome_13",
                "location": {"start": 31, "end": 39, "length": 9},
                "strand": {"value": 1},
            },
            "relative_location": {"start": 22, "end": 30, "length": 9},
            "checksum": None,
            "so_term": "intron",
        },
        {
            ...
            "slice": {
                "region_id": "test_genome_13",
                "location": {"start": 61, "end": 89, "length": 29},
                "strand": {"value": 1},
            },
            "relative_location": {"start": 52, "end": 80, "length": 29},
            "checksum": None,
            "so_term": "intron",
        },
    ]
    ...
    reverse_transcript = {"start": 10, "end": 100, "strand": -1}
    reverse_exons = [
        {
            "stable_id": "ENSE03.1",
            "slice": {
    ...
    target_reverse_introns = [
        {
            ...
            "relative_location": {"start": 12, "end": 40, "length": 29},
            "checksum": None,
            "so_term": "intron",
        },
        {
            ...
            "relative_location": {"start": 62, "end": 70, "length": 9},
            "checksum": None,
            "so_term": "intron",
        },
    ]
...
def test_cdna_formatting():
    ...
    transcript = {
        ...
        "start": 1,
        "end": 100,
        "exons": [{"start": 1, "end": 20}, {"start": 81, "end": 100}],
    }
    refget = RefgetDB()
...
def test_protein_formatting():
    ...
    refget = RefgetDB()
    result = format_protein(protein=protein, genome_id="tralalala", refget=refget)
        ...
        "family_matches": [
            {
                ...
                "score": 212.5,
                "evalue": 5.1e-63,
                "hit_location": {"start": 1, "end": 157, "length": 157},
            }
        ],
        "length": 10,
...
def test_relative_coords():
    ...
    rel_coords = calculate_relative_coords(
        parent_params={"start": 10, "end": 30, "strand": 1},
        child_params={"start": 15, "end": 26},
    )
    ...
    rel_coords = calculate_relative_coords(
        parent_params={"start": 1, "end": 10, "strand": -1},
        child_params={"start": 1, "end": 10},
    )

...
def create_biotypes(input_tsv, feature_type="transcript"):
    ...
    with open(input_tsv, encoding="UTF-8") as handle:
        ...
        for row in reader:
            biotype_description = row[
                "description (longer description that can contain nuances)"
            ]
            biotype_description_nullable = (
                None if biotype_description == "" else biotype_description
            )
            biotype_valuesets[row["value"]] = {
                "value": row["value"],
                "label": row["label"],
                "accession_id": row["Accession ID (Unique within Ensembl)"],
                "definition": row[
                    "definition of term (succinct - as few words as possible. )"
                ],
            ...
            if feature_type == "transcript" and row["value"] in TRANSCRIPT_NAME_MAPPING:
                biotype_valuesets[
                    TRANSCRIPT_NAME_MAPPING[row["value"]]
                ] = biotype_valuesets[row["value"]]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script for populating biotypes from https://docs.google.com/spreadsheets/d/1dwMV8xiIPKZrThdV5V1cPBusciEufFdq1PBBFRL05Lw/edit#gid=888403877"
    )
    parser.add_argument(
        "--input_tsv", help="File containing biotype info from the google doc"
    )

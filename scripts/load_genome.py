...
def load_genome_info(mongo_client, source_file):
    ...
    with open(source_file, encoding="UTF-8") as file:
        ...
        mongo_client.collection().insert_one(
            {
                "type": "Genome",
                "id": common.utils.get_genome_id(
                    doc["organism"]["name"], doc["assembly"]["accession"]
                ),
                "name": doc["assembly"]["default"],
...
def create_index(mongo_client):
    ...
    mongo_client.collection().create_index(
        [("scientific_name", pymongo.ASCENDING)], name="species_by_name"
    )
    mongo_client.collection().create_index(
        [("taxon_id", pymongo.ASCENDING)], name="species_by_taxon"
    )

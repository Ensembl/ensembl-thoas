...
def load_gene_info(
    ...
    with open(json_file, encoding="UTF-8") as file:
        ...
        for gene in ijson.items(file, "item"):
            ...
            gene_metadata = {
                "biotype": gene_biotype_classifiers.get(normalised_gene_biotype)
            }

            # Gene biotypes in data dumps may be missing the "_gene" suffix
            if not gene_metadata["biotype"]:
                gene_metadata["biotype"] = gene_biotype_classifiers.get(
                    normalised_gene_biotype + "_gene"
                )
            ...
            try:
                gene_metadata["name"] = common.utils.get_gene_name_metadata(
                    gene_name_metadata[gene["id"]], xref_resolver, logger
                )
            ...
            json_gene = {
                ...
                "name": re.sub(r"\[.*?\]", "", gene["description"]).rstrip()
            ...
            transcript_buffer = common.utils.flush_buffer(
                mongo_client, transcript_buffer
            )
...
def get_genome_assembly(assembly_name, mongo_client):
    assembly = mongo_client.collection().find_one(
        {"type": "Assembly", "name": assembly_name}
    )
    genome = mongo_client.collection().find_one(
        {"type": "Genome", "name": assembly_name}
    )
    if not genome or not assembly:
        raise IOError(
            f"Failed to fetch {assembly_name} assembly and genome info from MongoDB"
        )
...
def format_transcript(
    ...
    new_transcript = {
        ...
        "relative_location": common.utils.calculate_relative_coords(
            ...
            child_params={"start": transcript["start"], "end": transcript["end"]},
        ),
        "slice": common.utils.format_slice(
            region_name=region_name,
        ...
        "spliced_exons": common.utils.splicify_exons(
    ...
    if transcript["id"] in cds_info:
        ...
        cds_sequence_checksum, _ = refget.get_checksum_and_length(
            new_transcript["stable_id"], refget.cds
        )
        cds_sequence = common.utils.format_sequence(
            refget, cds_sequence_checksum, refget.cds
        )
            ...
            new_transcript["product_generating_contexts"].append(
                {
                    ...
                    "cdna": common.utils.format_cdna(
                        transcript=transcript, refget=refget
                    ),
                }
            )
        ...
        new_transcript["product_generating_contexts"].append(
            {
                ...
                "default": defaults.pop(),
                "cdna": common.utils.format_cdna(
                    transcript=transcript, refget=refget, non_coding=True
                ),
            }
        )
        ...
        for line in protein_file:
            ...
            protein_buffer.append(
                common.utils.format_protein(
                    protein=product, genome_id=genome_id, refget=refget
                )
            )
...
def preload_exon_phases(production_name, assembly):
    ...
    with open(
        production_name + "_" + assembly + "_phase.csv", encoding="UTF-8"
    ) as file:
...
def preload_transcript_meta(production_name, assembly):
    transcript_meta = {}
    with open(
        production_name + "_" + assembly + "_attrib.csv", encoding="UTF-8"
    ) as file:
        reader = csv.DictReader(file)
        for row in reader:
...
def preload_gene_name_metadata(production_name, assembly):
    gene_name_metadata = {}
    with open(
        production_name + "_" + assembly + "_gene_names.json", "r", encoding="UTF-8"
    ) as gene_name_metadata_file:
...
if __name__ == "__main__":
    ...
    if ARGS.log_faulty_urls:
        URL_LOGGER = ThoasLogging(
            logging_file=f'url_log_{GENOME["id"]}',
            logger_name=f'url_logger_{GENOME["id"]}',
        )

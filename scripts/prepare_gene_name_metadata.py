...
def retrieve_gene_name_metadata(sp_production_name, sp_assembly_name, mysql_cursor):
    ...
    extractor, backup_extractor = (
        [extract_info_from_description_column, extract_info_using_display_xref_id]
    ...
    with open(
        sp_production_name + "_" + sp_assembly_name + "_gene_names.json",
        "w",
        encoding="UTF-8",
    ) as write_file:
        ...
        if match:
            ...
            for element in matched_string_elements:
                ...
                if key == "Source":
                    ...
                    gene_name_info["external_db_display_name"] = gene_name_info[
                        "external_db_name"
                    ]
                if key == "Acc":
